  ...
  return <Popover open={open} onOpenChange={setOpen}>
    ...
    <PopoverContent align="end" className="w-64 gap-1">
      ...
      {([
        ...
        ['group', '组', '组合已有账号并配置权重'],
      ] as const).map(([kind, label, description]) => <Button key={kind} variant="ghost" aria-label={`添加${label}账号`} className="h-auto flex-col items-start gap-1 px-3 py-2" onClick={() => { setOpen(false); onSelect(kind) }}>
