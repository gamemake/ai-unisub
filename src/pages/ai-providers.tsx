...
export function AIProviders() {
  ...
  return <><PageHeader title="AI Provider" description="统一管理订阅账户、API 服务与调度组，配置客户端访问和成员权重。" action={<div className="flex flex-wrap gap-2"><Button variant="outline" nativeButton={false} role="link" render={<a href="#ai-catalog" />}>模型供应商</Button><AddAIProviderButton onSelect={setEditing} /></div>} />
...
export function AIProviderForm({ account, initialKind, onClose }: ({ account: Account; initialKind?: never } | { account?: undefined; initialKind: ProviderKind }) & { onClose: () => void }) {
  ...
  return <Modal wide title={`${account ? '编辑' : '添加'}${kinds.find(([value]) => value === kind)?.[1]} AI Provider`} description={kind === 'group' ? '组合已有 AI Provider；权重高者优先，同权重优先复用会话，否则随机选择。' : kind === 'api' ? '使用 API Key 连接模型服务，服务地址统一在模型供应商中配置。' : '绑定 Anthropic、OpenAI 或 Grok 订阅账户，由账户决定模型供应商。'} onClose={onClose}><form onSubmit={submit} className="space-y-5">
    {isGroup ? <>
      <div className="grid items-start gap-6 sm:grid-cols-2">
        <div className="space-y-4">
          ...
          <Field label="允许的客户端" hint="必须与所有组成员一致；Any 组仅允许 Any 成员。"><AppSelect value={client} onValueChange={value => { setClient(value as ClientType); setValidation(null) }}>{(['Any', 'Anthropic', 'OpenAI', 'Grok'] as ClientType[]).map(value => <SelectItem key={value} value={value}>{value}</SelectItem>)}</AppSelect></Field>
        </div>
        <section aria-label="组成员" className="min-w-0 space-y-3">
          ...
          <div className="overflow-hidden rounded-lg border">
            <UITable aria-label="组成员"><TableHeader><TableRow>
              ...
              return <TableRow key={p.id} className={member && allowedClient(p) !== client ? 'bg-destructive/10' : undefined}>
                ...
                <TableCell className="whitespace-normal px-2 py-3"><div className="break-words font-medium">{p.name}</div><div className="mt-1 text-xs text-muted-foreground">{allowedClient(p) === 'Any' ? '不限客户端' : allowedClient(p)}</div></TableCell>
          ...
          {!providers.isPending && !memberOptions.length && <p className="text-sm text-muted-foreground">暂无可选成员，请先创建订阅或 API 类型的 AI Provider。组不能嵌套。</p>}
        ...
        <div className={`grid items-start gap-6 ${kind === 'api' ? 'sm:grid-cols-3' : 'sm:grid-cols-2'}`}>
          ...
          <div className={kind === 'api' ? 'contents' : 'space-y-4'}>
            ...
            {kind === 'subscription' ? <>
              ...
              <Field label="允许的客户端" hint="单选；Any 包括未知客户端。"><AppSelect value={client} onValueChange={value => setClient(value as ClientType)}>{(['Any', 'Anthropic', 'OpenAI', 'Grok'] as ClientType[]).map(value => <SelectItem key={value} value={value}>{value}</SelectItem>)}</AppSelect></Field>
            </>}
            </div>
            <div className="space-y-4">
