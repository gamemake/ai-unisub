...
import { AIProviderForm } from '@/pages/ai-providers'
import { TableRow, TableCell } from '@/components/ui/table'
import { AppSelect } from '@/components/app-select'
import { SelectItem } from '@/components/ui/select'
import { useState } from 'react'
import { Activity, ArrowDownLeft, ArrowUpRight, KeyRound, Layers3 } from 'lucide-react'
import { useProviderOptions, useAIProviders, useCalls, useKeys, useUsage } from '@/data/store'
...
export function Personal({ me }: { me: User }) {
  ...
  const keys = useKeys(), accounts = useProviderOptions(), calls = useCalls({ page_size: '10', mine: '1' })
  return <><PageHeader title="个人总览" description={`你好，${me.name}。这里是你的 AI 工作空间。`} action={<Badge enabled>已登录</Badge>} />
    <div className="mb-6 grid gap-4 sm:grid-cols-3"><Stat label="我的 API Key" value={keys.data ? number(keys.data.total) : '—'} icon={KeyRound} detail="当前账号签发的调用密钥" /><Stat label="可选 AI Provider" value={accounts.data ? number(accounts.data.items.filter(a => a.enabled).length) : '—'} icon={Layers3} detail="处于启用状态的订阅、API 或组" /><Stat label="我的累计请求" value={calls.data ? number(calls.data.total) : '—'} icon={Activity} detail="已记录的历史调用" /></div>
...
export function Overview() {
  ...
  return <><PageHeader title="系统总览" description="查看所有 AI Provider与用户在选定时间范围内的调用用量。" />
    <div className="mb-6"><TimeRangeFilter onChange={setParams} /></div>
    <div className="mb-6 grid gap-4 sm:grid-cols-3"><Stat label="请求数" value={totals ? number(totals.requests) : '—'} detail="所选范围 · 全部 AI Provider" icon={Activity} /><Stat label="输入 Token" value={totals ? number(totals.input_tokens) : '—'} detail="上游返回的用量" icon={ArrowDownLeft} /><Stat label="输出 Token" value={totals ? number(totals.output_tokens) : '—'} detail="上游返回的用量" icon={ArrowUpRight} /></div>
    <div className="space-y-6"><Card><CardHeader><CardTitle>AI Provider 用量</CardTitle></CardHeader><QueryState query={subs}>{subs.data?.data.length ? <Table headers={['AI Provider', '平台', '请求', '输入 Token', '输出 Token']}>{subs.data.data.map((r, i) => { const provider = accounts.data?.items.find(a => a.id === r.subscription_id); return <DetailTableRow key={r.subscription_id || i} onOpen={provider ? () => setSelectedProvider(provider) : undefined}><TableCell>{r.subscription_name || r.subscription_id}</TableCell><TableCell>{r.provider}</TableCell><TableCell>{number(r.usage.requests)}</TableCell><TableCell>{number(r.usage.input_tokens)}</TableCell><TableCell>{number(r.usage.output_tokens)}</TableCell></DetailTableRow> })}</Table> : <Empty>所选范围内没有调用记录</Empty>}</QueryState></Card>
    <Card><CardHeader><div className="flex flex-wrap items-center justify-between gap-3"><CardTitle>用户用量</CardTitle><AppSelect aria-label="按 AI Provider 筛选用户用量" className="max-w-60" value={subscription} onValueChange={value => setSubscription(value)}><SelectItem value="">所有 AI Provider</SelectItem>{accounts.data?.items.map(a => <SelectItem key={a.id} value={String(a.id)}>{a.name}</SelectItem>)}</AppSelect></div></CardHeader><QueryState query={users}>{users.data?.data.length ? <Table headers={['用户', '角色', '请求', '输入 Token', '输出 Token']}>{users.data.data.map((r, i) => <TableRow key={r.user_id || i}><TableCell>{r.username || '未归属'}</TableCell><TableCell>{r.role || '—'}</TableCell><TableCell>{number(r.usage.requests)}</TableCell><TableCell>{number(r.usage.input_tokens)}</TableCell><TableCell>{number(r.usage.output_tokens)}</TableCell></TableRow>)}</Table> : <Empty>所选范围内没有调用记录</Empty>}</QueryState></Card></div>{selectedProvider && <AIProviderForm account={selectedProvider} onClose={() => setSelectedProvider(null)} />}
  </>
}
