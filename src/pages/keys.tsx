...
function KeyForm({ onClose, onCreated }: { onClose: () => void; onCreated: (key: APIKey) => void }) {
  ...
  return <Modal title="签发 API Key" onClose={onClose}><QueryState query={accounts}><form className="space-y-5" onSubmit={e => { e.preventDefault(); create.mutate({ name, account_id: Number(account), valid_seconds: days * 86400 }, { onSuccess: onCreated }) }}><Field label="名称"><Input required maxLength={64} value={name} onChange={e => setName(e.target.value)} placeholder="例如 Claude Code" /></Field><Field label="绑定 AI Provider"><AppSelect required value={account} onValueChange={value => setAccount(value)}><SelectItem value="">请选择 AI Provider</SelectItem>{accounts.data?.items.filter(a => a.enabled).map(a => <SelectItem key={a.id} value={String(a.id)}>{a.name} · {a.provider}</SelectItem>)}</AppSelect></Field><Field label="有效期（天）" hint="0 表示长期有效"><Input required type="number" min={0} max={36500} step={1} value={days} onChange={e => setDays(Number(e.target.value))} /></Field><ErrorMessage error={create.error} /><div className="flex justify-end"><Submit pending={create.isPending}>签发 Key</Submit></div></form></QueryState></Modal>
}
