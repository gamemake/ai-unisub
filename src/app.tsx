...
import { Activity, BookOpen, SlidersHorizontal, ChevronRight, Home, KeyRound, Layers3, LayoutDashboard, LogOut, Menu, Network, RefreshCw, Shield, Users as UsersIcon } from 'lucide-react'
...
const Suppliers = lazy(() => import('./pages/suppliers').then(module => ({ default: module.SuppliersPage })))
...
const navigation = [
  ...
  { id: 'overview', label: '系统总览', icon: LayoutDashboard, admin: true }, { id: 'accounts', label: '账号管理', icon: Layers3, admin: true },
  { id: 'suppliers', label: '模型供应商', icon: SlidersHorizontal, admin: true },
  { id: 'proxies', label: '代理管理', icon: Network, admin: true }, { id: 'users', label: '用户管理', icon: UsersIcon, admin: true },
  { id: 'api-docs', label: 'API 文档', icon: BookOpen, admin: true, href: '/api/docs' },
]
function pageFromHash() { const raw = location.hash.slice(1); if (raw.startsWith('suppliers/')) return 'suppliers'; return raw || 'personal' }
...
function Dashboard() {
  ...
  function navigate(id: string, href?: string) { if (href) { window.open(href, '_blank', 'noopener,noreferrer'); setMobileOpen(false); return }; location.hash = id; setPage(id); setMobileOpen(false) }
  const sidebar = <>      <div className="flex h-20 items-center gap-3 px-6"><span className="rounded-xl bg-primary p-2 text-primary-foreground"><Layers3 className="size-5" /></span><span className="font-semibold tracking-[.16em]">UNISUB</span></div>
      <p className="px-6 pb-3 pt-5 text-[10px] tracking-[.18em] text-muted-foreground">工作空间</p><nav className="space-y-1 px-3" aria-label="主导航">{items.map((n, i) => <div key={n.id}>{n.admin && !items[i - 1]?.admin && <p className="px-3 pb-3 pt-7 text-[10px] tracking-[.18em] text-muted-foreground">管理中心</p>}<Button variant="ghost" aria-current={active.id === n.id ? 'page' : undefined} onClick={() => navigate(n.id, n.href)} className={cn('h-auto w-full justify-start gap-3 rounded-lg px-3 py-3 text-sm transition-colors', active.id === n.id ? 'bg-primary/10 font-medium text-primary' : 'text-muted-foreground hover:bg-accent hover:text-foreground')}><n.icon className="size-[18px]" />{n.label}{active.id === n.id && <ChevronRight className="ml-auto size-4" />}</Button></div>)}</nav>
  ...
  return <div className="min-h-dvh lg:pl-60">
    ...
    <main className="mx-auto max-w-[1600px] p-5 lg:p-9"><ErrorMessage error={logout.error} /><QueryState query={me}><Suspense fallback={<div role="status" className="p-12 text-center text-muted-foreground">正在加载页面…</div>}>{me.data && <div key={active.id}>{active.id === 'personal' && <Personal me={me.data} />}{active.id === 'keys' && <Keys />}{active.id === 'logs' && <Logs me={me.data} />}{active.id === 'security' && <Security />}{admin && active.id === 'overview' && <Overview />}{admin && active.id === 'accounts' && <Accounts />}{admin && active.id === 'suppliers' && <Suppliers />}{admin && active.id === 'users' && <Users me={me.data} />}{admin && active.id === 'proxies' && <Proxies />}</div>}</Suspense></QueryState></main>
  </div>
}
