...
func NewWithDependencies(cfg Config, db database.Database, providers aiprovider.ProviderManager) (*Service, error) {
	...
	oauthManager := oauth.NewManager(db, proxyManager)
	if providers == nil {
		var err error
		providers, err = aiprovider.NewProviderManager(db, proxyManager, oauthManager)
		if err != nil {
...
func (s *Service) AIProviders() aiprovider.ProviderManager { return s.aiProviders }
func (s *Service) Proxy() proxy.ProxyManager               { return s.proxy }
