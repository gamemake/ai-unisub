...
func (*SupplierOpenAI) GetID() string   { return "openai" }
func (*SupplierOpenAI) GetName() string { return "OpenAI" }
...
func (s *SupplierOpenAI) authHeaders(config AccountConfig) (http.Header, error) {
	...
	if accountID := s.findString(credential, "account_id", "accountId"); accountID != "" {
		headers.Set("ChatGPT-Account-Id", accountID)
	}
	return headers, nil
}

func (s *SupplierOpenAI) findString(value any, names ...string) string {
...
func (s *SupplierOpenAI) doGET(ctx context.Context, account *Account, endpoint string, headers http.Header) ([]byte, error) {
	if ctx == nil {
		ctx = context.Background()
	}
	...
	for attempt := range attempts {
		selected, err := manager.proxy.ResolveProxy(ctx, config.ProxyGroupID, "openai", tried)
		if err != nil {
		...
		if err != nil {
			_ = manager.proxy.ReportProxyResult(selected, "openai", proxy.ApplicationIgnored, err, 0)
			return nil, err
		}
		...
		if requestErr != nil {
			...
			_ = manager.proxy.ReportProxyResult(selected, "openai", class, requestErr, 0)
		...
		_ = manager.proxy.ReportProxyResult(selected, "openai", class, readErr, resp.StatusCode)
