...
func (s *SupplierZAI) findString(value any, names ...string) string {
...
func (s *SupplierZAI) doGET(ctx context.Context, account *Account, endpoint string, headers http.Header) ([]byte, error) {
	...
	for attempt := range attempts {
		selected, err := manager.proxy.ResolveProxy(ctx, config.ProxyGroupID, "zhipu", tried)
		if err != nil {
		...
		if err != nil {
			_ = manager.proxy.ReportProxyResult(selected, "zhipu", proxy.ApplicationIgnored, err, 0)
			return nil, err
		}
		...
		if requestErr != nil {
			...
			_ = manager.proxy.ReportProxyResult(selected, "zhipu", class, requestErr, 0)
		...
		_ = manager.proxy.ReportProxyResult(selected, "zhipu", class, readErr, resp.StatusCode)
