...
func (*SupplierKimi) GetID() string   { return "kimi" }
func (*SupplierKimi) GetName() string { return "Kimi" }
...
func (s *SupplierKimi) findString(value any, names ...string) string {
...
func (s *SupplierKimi) doGET(ctx context.Context, account *Account, endpoint string, headers http.Header) ([]byte, error) {
	if ctx == nil {
		ctx = context.Background()
	}
	...
	for attempt := range attempts {
		selected, err := manager.proxy.ResolveProxy(ctx, config.ProxyGroupID, "kimi", tried)
		if err != nil {
		...
		if err != nil {
			_ = manager.proxy.ReportProxyResult(selected, "kimi", proxy.ApplicationIgnored, err, 0)
			return nil, err
		}
		...
		if requestErr != nil {
			...
			_ = manager.proxy.ReportProxyResult(selected, "kimi", class, requestErr, 0)
		...
		_ = manager.proxy.ReportProxyResult(selected, "kimi", class, readErr, resp.StatusCode)
