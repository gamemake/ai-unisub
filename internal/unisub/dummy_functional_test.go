...
func (p *dummyAIProvider) Handle(r *http.Request, recorder aiprovider.APICallRecorder) {
...
func TestDummyUserAIProviderAndAPIKeyFlow(t *testing.T) {
	...
	s, err := framework.NewWithDependencies(framework.Config{DatabaseURL: "sqlite::memory:"}, db, aiProviders)
	if err != nil {
		t.Fatal(err)
	}
	defer s.Close()
	...
	createdResponse := request(http.MethodPost, "/api/ai-providers", `{"name":"Dummy","provider":"dummy","config":{"name":"initial"}}`)
