...
func TestPersonalAPIPermissions(t *testing.T) {
	...
	for _, path := range []string{"/api/users", "/api/users/admin/password", "/api/ai-providers", "/api/ai-providers/id", "/api/providers", "/api/providers/id", "/api/ai-catalog", "/api/ai-catalog/kimi", "/api/proxy-groups", "/api/proxy-groups/errors", "/api/proxy-groups/test", "/api/usage/users", "/api/usage/subscriptions", "/api/oauth/dummy/start", "/api/oauth/dummy/status/id", "/api/oauth/dummy/poll/id", "/api/oauth/dummy/complete/id", "/api/oauth/results/id"} {
