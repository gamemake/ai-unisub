...
func TestDecodeAIProviderConfigDefaultsAndProxy(t *testing.T) {
	...
	disabled, err := decodeAIProviderConfig("p1", json.RawMessage(`{"enabled":false,"proxy":"socks5://127.0.0.1:1080","max_concurrent_connections":4,"queue_timeout_seconds":15}`))
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := decodeAIProviderConfig("p1", json.RawMessage(`{"proxy":"ftp://127.0.0.1:21"}`)); err == nil {
