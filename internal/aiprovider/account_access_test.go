...
func TestAccountGetAccessLoadsOAuthCredential(t *testing.T) {
	...
	account := &Account{
		...
		Config: AccountConfig{
			Kind:       AccountSubscription,
			Supplier:   "openai",
			Credential: oauth.OAuthCredential{AccessToken: "oauth-token"},
		},
	}
...
func TestAccountGetAccessRefreshesStaleOAuthCredential(t *testing.T) {
	...
	manager := &providerManager{db: db, oauth: oauth.NewManager(db, proxyManager)}
	...
	account := &Account{
		...
		Config: AccountConfig{
			Kind:        AccountSubscription,
			Supplier:    oauth.OAuthServiceDummy,
