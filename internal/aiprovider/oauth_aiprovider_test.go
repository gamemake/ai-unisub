...
func TestOAuthAIProvidersHandleAPICalls(t *testing.T) {
	...
	p, err := NewClaudeAIProvider("provider-1", json.RawMessage(`{"credential_id":"credential-1"}`), manager)
	if err != nil {
		t.Fatal(err)
	}
	...
	p.Handle(req, func(value *AIProviderCallTrace) { trace = value })
	if trace == nil {
...
func (a *testOAuthAdapter) Refresh(_ context.Context, credential *oauth.OAuthCredential, _ *http.Client) (*oauth.OAuthCredential, error) {
