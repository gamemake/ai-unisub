...
func TestProviderManagerGatewayEndToEnd(t *testing.T) {
	...
	proxyGroupID, err := proxyManager.Create(proxy.ProxyGroupConfig{Name: "gateway proxies", Proxies: []string{upstream.URL}})
	if err != nil {
		t.Fatal(err)
	}
	...
	if proxyTotal != 1 || len(proxyLogs) != 1 || proxyLogs[0].AppType != "openai" || proxyLogs[0].HTTPErrorCode != http.StatusOK {
		t.Fatalf("proxy logs = %+v, total = %d", proxyLogs, proxyTotal)
	}
}
