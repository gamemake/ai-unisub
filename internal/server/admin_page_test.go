...
func TestAdminPageIsServedWithoutCaching(t *testing.T) {
	...
	for _, marker := range []string{"UniSub", "账号管理", "accountTable", "accountModalTitle", "page-keys", "page-logs", "page-users", "page-security", "apiKeyForm", "userForm", "用户管理", "安全设置", "调用记录", "grokOAuthModal", "logModal", "modal-shell fullscreen", "modal fullscreen", "/assets/admin.css", "/assets/admin.js"} {
...
func TestAdminAssetsAreEmbeddedAndServed(t *testing.T) {
	...
	tests := []struct {
		...
		{path: "/assets/admin.js", contentType: "application/javascript", markers: []string{"/api/accounts", "/api/api-keys", "/api/request-logs", "sessionStorage", "accountsNav", "usersNav", "renderAccounts", "formatLocalTokens", "renderAPIKeys", "renderLogs", "formatDuration", "created_by_user_id===currentUser.id", "renderQuota", "creditBalanceLine", "usagePercent", "refreshData", "usage/refresh", "<th>Usage</th>", "<th>Token</th>", "openEditAccount", "prettyJSON", "data-action=\"edit\"", "data-action=\"delete\"", "编辑账号", "ccswitch://v1/import", "openCCSwitch", "launchCCSwitch", "apiKey:plaintext", "grokbuild", "/api/users", "renderUsers", "/api/providers/grok/oauth/device/start", "/api/providers/grok/oauth/device/poll", "Request Headers", "Response Headers", "data-copy-index", "data-log-toggle", "formatLogHeaders", "缓存读", "缓存写", "缓存创建", "缓存读取", "输入 ", "合计 ", "总数 ", "<th>ID</th>", "<th>模型</th>", "日志 ID", "来源 IP", "client_ip", " 秒", " 分钟"}},
	}
