...
func TestAdminAssetsAreEmbeddedAndServed(t *testing.T) {
	...
	tests := []struct {
		...
		{path: "/assets/admin.css", contentType: "text/css", markers: []string{".login-shell", ".account-name", ".nav #overviewNav:before", ".log-pre", ".log-block-head", ".log-block.collapsed", ".modal.fullscreen", "grid-template-columns:78px minmax(0,1fr)", ".oauth-paste", ".actions .icon-btn", ".table-card>.card-head", "#userUsageSubscriptionFilter", "flex:0 0 200px", ".table-pager select", ".usage-query-icon svg", ".usage-sort", ".usage-sort-mark"}},
		{path: "/assets/admin.js", contentType: "application/javascript", markers: []string{"/api/subscriptions", "/api/api-keys", "/api/request-logs", "/api/usage/by-subscription", "/api/usage/by-user", "renderUsageByUser", "loadUsageByUser", "setUserUsageRange", "userUsageRange", "userUsageSubscriptionFilter", "fillUserUsageSubscriptionOptions", "userUsageQuery", "userUsageLimit", "userUsageOffset", "usageByUserPager", "changeLogPageSize", "changeUserUsagePageSize", "syncLogPageSize", "pageSizeSelectHTML", "logPageSize", "userUsagePageSize", "subscription_id", "sessionStorage", "overviewNav", "accountsNav", "usersNav", "logsNav", "$('logsNav').hidden=false", "renderPersonalOverview", "personalRecentCalls", "personalStatKeys", "landingResolved", "renderAccounts", "formatLocalTokens", "renderAPIKeys", "renderLogs", "logKeyLabel", "用户 '+escapeHTML(username)", "logMetaItem('用户'", "formatDuration", "logAccountFilter", "fillLogFilters", "renderQuota", "creditBalanceLine", "usagePercent", "refreshData", "usage/refresh", "loadUsageBySubscription", "usageSortHeader", "sortUsageRows", "usageSortClick", "data-usage-sort-key", "aria-sort", "<th>Usage</th>", "<th>Token</th>", "openEditAccount", "prettyJSON", "iconActionButton('edit'", "iconActionButton('delete'", "iconActionButton('refresh-usage'", "iconActionButton('ccswitch'", "canRefreshUsage", "刷新 Usage", "iconActionButton", "actionIcon", "'编辑'", "'删除'", "编辑账号", "ccswitch://v1/import", "openCCSwitch", "launchCCSwitch", "apiKey:plaintext", "grokbuild", "/api/users", "renderUsers", "reset-password", "/password", "openResetPasswordModal", "saveResetPassword", "/api/providers/grok/oauth/device/start", "/api/providers/grok/oauth/device/poll", "/api/providers/claude/oauth/start", "/api/providers/codex/oauth/exchange", "登录并绑定 Claude", "登录并绑定 Codex", "startPKCEOAuth", "submitPKCEOAuth", "pkceOAuthModal", "Request Headers", "Response Headers", "data-copy-index", "data-log-toggle", "formatLogHeaders", "缓存创建 ", " · 缓存读取 ", "输入 ", "合计 ", "总数 ", "<th>ID</th>", "<th>模型</th>", "日志 ID", "来源 IP", "client_ip", " 秒", " 分钟"}},
	}
