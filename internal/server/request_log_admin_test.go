...
func TestAdminRequestLogsCollection(t *testing.T) {
	...
	list := httptest.NewRequest(http.MethodGet, "/admin/request-logs", nil)
	...
	detail := httptest.NewRequest(http.MethodGet, "/admin/request-logs/"+listed.Data[0].Day+"/"+strconv.FormatInt(listed.Data[0].ID, 10), nil)
