...
import (
	aiprovider "ai-unisub/internal/aiprovider2"
	"ai-unisub/internal/common"
	"ai-unisub/internal/database"
	proxy "ai-unisub/internal/proxy2"
...
func publicProviderAccount(account *aiprovider.Account, admin bool) map[string]any {
	item := map[string]any{
		...
		"auth_type": account.Config.Kind, "enabled": account.Config.Enabled,
	}
	if admin {
		item["config"] = account.Config
		if account.Config.Kind != aiprovider.AccountGroup {
			item["quota"] = account.Quota
		}
	}
	return item
}
...
func (m *APIModule) createAIProvider(ctx framework.ModuleContext, w http.ResponseWriter, r *http.Request) {
	...
	config, err := accountConfigJSON(input.Name, input.AIProvider, input.Config)
	if err != nil {
		common.WriteError(w, http.StatusBadRequest, common.MessageInvalidAccountConfig)
		return
	}
	account, err := ctx.AIProviders().NewAccount(config)
	if err != nil {
		common.WriteError(w, http.StatusBadRequest, common.MessageInvalidAccountConfig)
		return
	}
...
func (m *APIModule) updateAIProvider(ctx framework.ModuleContext, w http.ResponseWriter, r *http.Request, id int) {
	...
	if input.AIProvider != "" && input.AIProvider != string(account.Config.Kind) {
		common.WriteError(w, 400, common.MessageAIProviderTypeCannotChange)
		return
	}
	if len(input.Config) == 0 || string(input.Config) == "null" {
		input.Config, _ = json.Marshal(account.Config)
	}
	config, err := accountConfigJSON(input.Name, string(account.Config.Kind), input.Config)
	if err != nil || ctx.AIProviders().SetAccountConfig(r.Context(), id, config) != nil {
		common.WriteError(w, http.StatusBadRequest, common.MessageInvalidAccountConfig)
		return
	}
...
func accountConfigJSON(name, provider string, raw json.RawMessage) (json.RawMessage, error) {
	...
	if config.Kind == "" {
		config.Kind = aiprovider.AccountKind(provider)
	}
...
func accountClients(ctx framework.ModuleContext, accountID int) []aiprovider.ClientType {
	...
	if account == nil || account.Config.ClientType == "" {
		return nil
	}
	return []aiprovider.ClientType{account.Config.ClientType}
}
