...
除 `/v1/` 网关转发外，管理端 `POST /api/accounts/{id}/refresh-quota` 与 `POST /api/accounts/{id}/fetch-models` 触发的上游 usage／models 查询也会写入调用记录，归属被查询的账号；`url` 与 `outbound_url` 均为上游地址。这类记录的原始与出站 request headers 均保存脱敏后的出站请求头（不单独保留浏览器管理请求头）。
